...
func (this *Session) _sql_with() string {
	...
	if len(this.withs) > 0 {
		...
		for k := range wmap {
			kk := tools.TextTemplate(fmt.Sprintf("{{ tmp_%s .ctx }}", k), map[string]any{"ctx": this.Ctx()}).FuncMap(this.tmpWiths).String()
			withs = append(withs, fmt.Sprintf("__sql_with_%s as (%s)", k, kk))
		}

		sqlwith = with_header + strings.Join(withs, ",")
		sqlwith = tools.TextTemplate(sqlwith, map[string]any{"ctx": this.Ctx()}).FuncMap(this.tmpWiths).String()
	}
	return sqlwith
}
